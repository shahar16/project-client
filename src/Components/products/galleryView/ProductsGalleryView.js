...
function ProductsGalleryView({ fetchService, renderStore, editMode }) {
	...
	return (
		<div>
			{productsList &&
				...
				>
					{productsList.map((items, index) => <ProductsGroup items={items} key={`${componentId}:${index}`} renderStore={renderStore} editMode={editMode}/>)}
				</InfiniteScroll>
			}
		</div >
	)
